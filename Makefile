...
build-img:
	docker build --tag rookout/todo-demo-java-multi-infra:latest --tag rookout/todo-demo-java-multi-infra:${PUBLISH_VERSION} .

upload-no-latest:
	docker push rookout/todo-demo-java-multi-infra:${PUBLISH_VERSION}

upload: upload-no-latest
	@if [ ${CIRCLE_BRANCH} = "master" ]; then \
		docker push rookout/todo-demo-java-multi-infra:latest; \
	fi
